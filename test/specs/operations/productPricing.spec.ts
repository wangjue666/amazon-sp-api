...
describe(endpoint, async function () {
  ...
  it('should return pricing information for asins array', async function () {
    if (this.config.asin2) {
      const asins = [this.config.asin, this.config.asin2];
      const res: any = await this.sellingPartner.callAPI({
        operation: 'getPricing',
        endpoint: endpoint,
        query: {
  ...
  it('should return pricing information for skus array', async function () {
    if (this.config.sku2) {
      const skus = [this.config.sku, this.config.sku2];
      const res: any = await this.sellingPartner.callAPI({
        operation: 'getPricing',
        endpoint: endpoint,
        query: {
  ...
  it('should return featured buying options as batch request', async function () {
    ...
    expect(res.responses[0].body.marketplaceId).to.equal(this.config.marketplace_id);
